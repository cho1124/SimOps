...
후속 개선: [Unity 게임 화면 개선](unity-ui-refresh.md) — 한국어 전투·보상·결과 UI, 수동 제출 상태, 덮어쓰기 보호, 렌더 검사.

클라이언트 확장: [WebGL 프로토타입](web-prototype.md) — 브라우저 플레이, 압축 빌드, 동일 출처 플레이어 API, 브라우저 저장 및 랭킹 검증.
